    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-8 md:p-12">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <TaskStatsCard label="Total de Tarefas" value={stats.total} color="slate" />
          <TaskStatsCard label="Concluídas" value={stats.completed} color="emerald" />
          <TaskStatsCard label="Pendentes" value={stats.pending} color="amber" />
        </section>

        {/* Formulário + Lista */}
        <section className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl space-y-6">
          <h2 className="text-lg font-semibold text-white">Criar Nova Tarefa</h2>
          ...
          {isLoading ? (
            <p className="text-slate-500 text-sm text-center py-8">Carregando tarefas do PostgreSQL...</p>
          ) : isError ? (
            <p className="text-rose-400 text-sm text-center py-8">Erro ao conectar com o servidor Fastify.</p>
          ) : (
